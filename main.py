...
def main():
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # train for one epoch
        train_loss, train_prec1, train_prec5, iff, ic2i = train(
        ...
        results.add(epoch=epoch + 1, train_loss=train_loss, val_loss=val_loss,
                    train_top1=train_prec1, val_top1=val_prec1,
                    train_top5=train_prec5, val_top5=val_prec5, iff = iff.item(), ic2i=ic2i.item())
...
def forward(data_loader, model, criterion, epoch=0, training=True,  bin_optimizer=None,fp_optimizer=None):
    ...
    top1 = AverageMeter()
    top5 = AverageMeter()
    iff = torch.Tensor([0]).cuda()
    counts = 0
    ic2i = torch.Tensor([0]).cuda()
    ...
    for i, (inputs, target) in enumerate(data_loader):
        ...
        if training:
            ...
            for p in list(model.parameters()):
                if hasattr(p,'org'):
                    #print("counts",p.data.numel())
                    #print(p.data)
                    #print(p.pre_binary_data)
                    iff += torch.sum(abs(torch.sign(p.data)-p.pre_binary_data)/2)
                    counts += p.data.numel()
                    if epoch==199:
                        ic2i += torch.sum(abs(torch.sign(p.data)-p.init_binary_data)/2)
    ...
    if training:
        return losses.avg, top1.avg, top5.avg, 1.0*iff/counts, 1-0.5*ic2i/counts
