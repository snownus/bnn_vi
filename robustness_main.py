...
def save_losses(dataset, sgd_train_losses, adam_train_losses, SDP_train_losses, 
                sgd_test_losses, adam_test_losses, SDP_test_losses):
    data = {
    ...
    df.to_csv(dataset + '_train1.csv', index=False)
    ...
    df = pd.DataFrame(data)
    df.to_csv(dataset + '_test1.csv', index=False)
...
if __name__ == '__main__':
    ...
    for dataset, dataloader in zip(['complete, random'], [complete_data, random_data]):
        ...
        sgd_optimizer = torch.optim.SGD(BASE1.parameters(), lr=0.01)
        ...
        adam_optimizer = torch.optim.SGD(BASE1.parameters(), lr=0.01)
        ...
        sdp_optimizer = torch.optim.SGD(SDP_model.parameters(), lr=0.01)
        ...
        df = pd.DataFrame(data)
        df.to_csv(dataset + '_hessian1.csv', index=False)