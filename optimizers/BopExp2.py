...
class BopExp2(Optimizer):
    def __init__(
        ...
        **kwargs
        ...
        super(BopExp2, self).__init__(params, defaults)
        ...
        for group in self.param_groups:
            for p in group['params']:
                ...
                if hasattr(p,'org'):
                    ...
                    # delta = delta/(threshold*(p.each_flip_num[indices]+ 1e-20))
                    ...
                    indices = torch.where((torch.sign(m_t) == torch.sign(p.data)) & (delta>=threshold*p.each_flip_num))
                    # indices = torch.where((torch.sign(m_t) == torch.sign(p.data)))
                    # print("indices: ",len(indices))
                    # print("ratio: ",len(indices)/p.data.numel())
                    delta = delta[indices]/threshold
