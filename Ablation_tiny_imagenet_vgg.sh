...
do
  ...
  nohup python main_sdp1_wo_gaussian_qudrature.py --model vgg_tiny_imagenet_sdp \
  --save vgg_tiny_imagenet_seed=${seed}_benchmark_K=${K}_S=${S}_L=${L}_lr=${lr}_milestones=60_120_180 \
  --dataset tiny_imagenet --binarization  det --wd ${wd} --lr ${lr} \
