...
CUDA_VISIBLE_DEVICES=4,5,6,7 nohup torchrun --master_port 10087 --nproc_per_node 4 main_imagenet.py -a resnet18_1w1a_recu --dali_cpu \
-save imagenet_birealnet_recu_benchmark_lr=${lr}_wd=${wd}_K=${K}_S=${S}_L=${L}_lr-decay=${lr_decay}_${binary_opt}_epochs=200  --wd ${wd} --lr ${lr} \
--epochs 200 -b 256 -j 20 -K $K -L $L -scale $S --lr_decay cos --binary_opt ${binary_opt} /home/gengxue/data/imagenet \
> /dev/null 2>&1 &
