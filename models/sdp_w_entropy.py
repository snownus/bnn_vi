...
class BinarizeConv2dSDP(nn.Module):
    ...
    def __init__(self, K, in_chn, out_chn, dropout=0, kernel_size=3, stride=1, padding=1, bias=False, linear=False):
        ...
        # self.scale = 1000000
        self.register_buffer('scale', torch.ones(1, requires_grad=False) * 1e8)
        ...
        self.register_buffer('lambdaa', torch.ones(1, requires_grad=False) * 0)
    ...
    def forward(self, input):
        ...
        with torch.no_grad():
            ...
            # print(f'z.data: {z.data}, A.data: {A}')
