...
class BinaryNet(nn.Module):
    ...
    def __init__(self, K, scale, variance, input_size=32, num_classes=100):
        ...
        self.features = nn.Sequential(
            ...
            BinarizeConv2dSDP(K, scale, 128*self.infl_ratio, 128*self.infl_ratio, kernel_size=3, padding=1, bias=False),
            nn.MaxPool2d(kernel_size=2, stride=2),
            nn.BatchNorm2d(128*self.infl_ratio),
            nn.Hardtanh(inplace=True),

            BinarizeConv2dSDP(K, scale, 128*self.infl_ratio, 256*self.infl_ratio, kernel_size=3, padding=1, bias=False),
            nn.BatchNorm2d(256*self.infl_ratio),
            nn.Hardtanh(inplace=True),


            BinarizeConv2dSDP(K, scale, 256*self.infl_ratio, 256*self.infl_ratio, kernel_size=3, padding=1, bias=False),
            nn.MaxPool2d(kernel_size=2, stride=2),
            nn.BatchNorm2d(256*self.infl_ratio),
            nn.Hardtanh(inplace=True),


            BinarizeConv2dSDP(K, scale, 256*self.infl_ratio, 512*self.infl_ratio, kernel_size=3, padding=1, bias=False),
            nn.BatchNorm2d(512*self.infl_ratio),
            nn.Hardtanh(inplace=True),


            BinarizeConv2dSDP(K, scale, 512*self.infl_ratio, 512, kernel_size=3, padding=1, bias=False),
            nn.MaxPool2d(kernel_size=2, stride=2),
            nn.BatchNorm2d(512),
            nn.Hardtanh(inplace=True)
        ...
        self.classifier = nn.Sequential(
            ...
            BinarizeConv2dSDP(K, scale, 512 * int(self.input_size/8) * int(self.input_size/8), 1024, kernel_size=1, padding=0, linear=True, bias=False),
            nn.BatchNorm1d(1024),
            nn.Hardtanh(inplace=True),
            # nn.Dropout(0.5),
            #BinarizeLinear(1024, 1024, bias=False),
            BinarizeConv2dSDP(K, scale, 1024, 1024, kernel_size=1, padding=0, linear=True, bias=False),
            nn.BatchNorm1d(1024),
            nn.Hardtanh(inplace=True),
            #nn.Dropout(0.5),
