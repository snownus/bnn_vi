...
def Binaryconv3x3(K, scale, in_planes, out_planes, stride=1):
    "3x3 convolution with padding"
    return BinarizeConv2dSDP(K, scale, in_planes, out_planes, kernel_size=3, stride=stride,
...
class BasicBlock(nn.Module):
    ...
    def __init__(self, K, scale, inplanes, planes, stride=1, downsample=None,do_bntan=True):
        super(BasicBlock, self).__init__()

        self.conv1 = Binaryconv3x3(K, scale, inplanes, planes, stride)
        self.bn1 = nn.BatchNorm2d(planes)
        self.tanh1 = nn.Hardtanh(inplace=True)
        self.conv2 = Binaryconv3x3(K, scale, planes, planes)
    ...
    def forward(self, x):
        ...
        out = self.conv2(out)
        #print("out",out.shape)
        #print("residual1",residual.shape)
...
class Bottleneck(nn.Module):
    ...
    def __init__(self, K, scale,  inplanes, planes, stride=1, downsample=None):
        ...
        self.conv1 = BinarizeConv2dSDP(K, scale, inplanes, planes, kernel_size=1, bias=False)
        self.bn1 = nn.BatchNorm2d(planes)
        self.conv2 = BinarizeConv2dSDP(K, scale, planes, planes, kernel_size=3, stride=stride,
                               paddSDPing=1, bias=False)
        self.bn2 = nn.BatchNorm2d(planes)
        self.conv3 = BinarizeConv2dSDP(K, scale, planes, planes * 4, kernel_size=1, bias=False)
...
class ResNet(nn.Module):
    ...
    def _make_layer(self, K, scale, block, planes, blocks, stride=1,do_bntan=True):
        downsample = None
        if stride != 1 or self.inplanes != planes * block.expansion:
            downsample = nn.Sequential(
                BinarizeConv2dSDP(K, scale, self.inplanes, planes * block.expansion,
        ...
        layers.append(block(K, scale, self.inplanes, planes,do_bntan=do_bntan))
...
class ResNet_imagenet(ResNet):
    ...
    def __init__(self, K, scale,  num_classes=1000,
        ...
        self.inplanes = 64
        self.conv1 = BinarizeConv2dSDP(3, 64, kernel_size=7, stride=2, padding=3,
                               bias=False)
        print(self.conv1)
...
def resnet_binary_sdp(**kwargs):
    ...
    if dataset == 'imagenet':
        ...
        return ResNet_cifar(K=K, scale=scale, num_classes=num_classes,
        ...
        K = kwargs.get( 'K', 2)
        scale = kwargs.get('scale', 100)
        return ResNet_tiny_imagenet(K=K, scale=scale, num_classes=num_classes,
                              block=BasicBlock, depth=depth)